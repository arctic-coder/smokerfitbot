...
class TestRecurringRetries(unittest.TestCase):
    def test_failed_retries_and_cancel(self):
        async def run():
            ...
            notifier = AsyncMock()
            with (
                patch("billing.service.get_subscription", new=AsyncMock(return_value=sub1)),
                patch("billing.service.get_last_pending_payment_id", new=AsyncMock(return_value=None)),
                patch(
                    "billing.service.create_recurring_payment",
                    new=AsyncMock(
                        return_value=types.SimpleNamespace(
                            id="p1",
                            status="failed",
                            amount=types.SimpleNamespace(value="399.00", currency="RUB"),
                            json=lambda: "{}",
                        )
                    ),
                ),
            ...
            notifier.reset_mock()
            with (
                patch("billing.service.get_subscription", new=AsyncMock(return_value=sub2)),
                patch("billing.service.get_last_pending_payment_id", new=AsyncMock(return_value=None)),
                patch(
                    "billing.service.create_recurring_payment",
                    new=AsyncMock(
            ...
            notifier.reset_mock()
            with (
                patch("billing.service.get_subscription", new=AsyncMock(return_value=sub3)),
                patch("billing.service.get_last_pending_payment_id", new=AsyncMock(return_value=None)),
                patch(
                    "billing.service.create_recurring_payment",
                    new=AsyncMock(
    ...
    def test_success_resets_retry_and_notifies(self):
        async def run():
            ...
            notifier = AsyncMock()
            with (
                patch("billing.service.get_subscription", new=AsyncMock(return_value=sub)),
                patch("billing.service.get_last_pending_payment_id", new=AsyncMock(return_value=None)),
                patch(
                    "billing.service.create_recurring_payment",
                    new=AsyncMock(
                        return_value=types.SimpleNamespace(
                            id="p_ok",
                            status="succeeded",
                            amount=types.SimpleNamespace(value="2990.00", currency="RUB"),
                            json=lambda: "{}",
                        )
                    ),
                ),
